...
// Broadcasts a Nostr event for stream updates
func BroadcastNostrUpdateEvent(metadataFile string) {
	...
	streamURL, ok := meta["stream_url"].(string) // Corrected key and type
	if !ok {
	...
	if err != nil {
		log.Printf("Error creating update event: %v", err)
		return
	}
